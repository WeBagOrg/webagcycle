$().ready(function() {
	...
	$("input[name='m:me_lyhtqd:hth']").change(function(){
		...
		if(result.isSuccess == 1) $.ligerDialog.error(result.msg,"出错了！");
...
function validatePuweiIsUsed(){
	 var startDate = $.trim($("#htyxqq").text());
	 var errMsg = "";
	 var warnMsg = "";
	 $("[name='s:me_lyhtqd_pwxx:pwID']").each(function(){	
		var pwID = $(this).val();
		var conf ={aliasName:'validatePuweiIsUsed',pwid:pwID,sDate:startDate};
