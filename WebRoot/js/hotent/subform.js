	...
	var subform={
			...
			addBind: function( table) {
				...
				$(".add",table).click(function(){
					self.add(table,null);//调用add方法，添加子记录
				});
			},
			...
			addRow:function(data,showData,table){
				...
				for(var name in data){
					tr.find("td,input[type='hidden']").each(function(){
						if($(this).is('td')){
			...
			editRow:function(obj,form){
				...
				var showData = self.getFormShowData(form);
				for(var name in data){
					$(obj).find("td,input[type='hidden']").each(function(){
						if($(this).is('td')){
							var tdname=$(this).attr('name');
							if(name==tdname){
								$(this).text(showData[name]);
			...
			setFormData:function(obj,table){
				...
				for(var name in json){
					form.find('input:text,input[type="hidden"],textarea,select').each(function() {
