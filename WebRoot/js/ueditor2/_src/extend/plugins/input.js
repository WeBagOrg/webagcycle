...
UE.plugins['input'] = function() {
	var me = this,
     ...
     optionHtml = ['<div id="panel" class="s">',
                   ...
                   '<td><div class="editable-left-div" ><label style="padding-left:5px">值: <input name="optionKey" type="text" class="short"/></label>',
                   '<label style="padding-left:5px">选项: <input name="optionValue" class="long" style="width:145px" type="text"/></label></div></td>',
                   '<td><div class="editable-right-div"><a href="###" class="remove-bnt" tabindex="-1">移除</a></div></td>',
                   '</tr></tbody></table></div></div>'].join('');
	
	radioHtml = ['<div id="panel" class="s">',
                  '<table id="option-table">',
                  '<thead><tr><td colspan="2"><a href="###" class="link add">添加</a></td></tr></thead>',
                  '<tbody></tbody></table>',
                  '<div class="hidden"><table id="hiddenTable"><tbody><tr class="editable-tr">',
                  '<td><div class="editable-left-div" ><label style="padding-left:5px">值: <input name="optionKey" type="text" class="short"/></label>',
                  '<label style="padding-left:5px">选项: <input name="optionValue" class="long" style="width:145px" type="text"/></label><label style="padding-left:5px">默认: <input style="margin-bottom:7px" name="isDefault" value="1" type="radio"/></label></div></td>',
                  '<td><div class="editable-right-div"><a href="###" class="remove-bnt" tabindex="-1">移除</a></div></td>',
                  '</tr></tbody></table></div></div>'].join('');
	checkboxHtml = ['<div id="panel" class="s">',
                 '<table id="option-table">',
                 '<thead><tr><td colspan="2"><a href="###" class="link add">添加</a></td></tr></thead>',
                 '<tbody></tbody></table>',
                 '<div class="hidden"><table id="hiddenTable"><tbody><tr class="editable-tr">',
                 '<td><div class="editable-left-div" ><label style="padding-left:5px">值: <input name="optionKey" type="text" class="short"/></label>',
                 '<label style="padding-left:5px">选项: <input name="optionValue" class="long" style="width:145px" type="text"/></label><label style="padding-left:5px">默认: <input style="margin-bottom:7px" name="isDefault" value="1" type="checkbox"/></label></div></td>',
                 '<td><div class="editable-right-div"><a href="###" class="remove-bnt" tabindex="-1">移除</a></div></td>',
                 '</tr></tbody></table></div></div>'].join('');
	//添加按钮
	...
	me.commands['checkbox'] = {
			execCommand : function(cmdName,data,old) {
				if(data){
					...
					for(var i=0,c;c=data[i++];){
						if(c.id=='options'){
							...
							for(var j=0,m;m=options[j++];){
								if(m.isDefault=="1"){
									text.push('<label><input type="checkbox" checked="checked" value="'+m.key+'"/>'+m.value+'</label>');
								}else{
					...
					me.curInputHtml=null;
					var html='<tr><th>复选框选项:</th><td colspan="3">'+checkboxHtml+'</td></tr>';
					me.curInputHtml=getContentHtml(html);
					initDialog(cmdName);
				}				
			},
				...
				if(data){
					...
					for(var i=0,c;c=data[i++];){
						if(c.id=='options'){
							...
							for(var j=0,m;m=options[j++];){
								if(m.isDefault=="1"){
									text.push('<label><input type="radio" checked value="'+m.key+'"/>'+m.value+'</label>');
								}else{
					...
					me.curInputHtml=null;
					var html='<tr><th>单选按钮选项:</th><td colspan="3">'+radioHtml+'</td></tr>';
					me.curInputHtml=getContentHtml(html);
					initDialog(cmdName);
				}
			},
...
//					html+='<tr><th>下拉框选项</th><td colspan="3"><span style="color:green;">每行对应一个选项<br /></span><textarea eid="options" cols="50" rows="5"></textarea></td></tr>';
			...
			execCommand : function(cmdName,data,old) {
				if(data){
					if(old){
						...
						insertControl(cmdName,data,'<textarea class="myeditor"></textarea>');
					}
				}
				else{
					dataType=[
	...
	function getContentHtml(html){
		var content=['<table class="edit_table"><tr><th>字段注释:</th><td><input eid="comment" type="text" validate="{required:true}"  onblur="getKeyName(this)"/></td><th>字段名称:</th><td><input eid="name" type="text" validate="{required:true,varirule:true}"/></td></tr><tr><th>数据类型:</th><td>'];
