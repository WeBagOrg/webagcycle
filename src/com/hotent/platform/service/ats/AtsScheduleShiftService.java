...
public class AtsScheduleShiftService extends BaseService<AtsScheduleShift> {
	...
	public Map<String, AtsScheduleShift> getByFileIdStartEndTimeMap(
		...
		for (AtsScheduleShift atsScheduleShift : list) {
			...
			map.put(DateFormatUtil.formatDate(atsScheduleShift.getAttenceTime()),
					atsScheduleShift);
		}
		return map;
	}
	...
	public void importExcel(InputStream inputStream, Short holidayHandle,
			String fileExt, int template) throws Exception {
	...
	private void handrimportExceltemplate2(HSSFSheet st, Short holidayHandle)
			throws Exception {
		// 第一行为标题，不取
		for (int rowIndex = 1; rowIndex <= st.getLastRowNum(); rowIndex++) {
			...
			for (int columnIndex = 0; columnIndex <= row.getLastCellNum(); columnIndex++) {
				...
				if (BeanUtils.isEmpty(o)
					...
					MsgUtil.addMsg(MsgUtil.WARN, "第" + (rowIndex + 1) + "行,"
							+ v + "为空值,该行导入失败");
				...
				if (columnIndex == 0) {// 考勤编号
					...
					if (BeanUtils.isEmpty(file)) {
						MsgUtil.addMsg(MsgUtil.ERROR, "第" + (rowIndex + 1)
								+ "行,找不到该员工编号:" + o + ",该行导入失败");
						atsScheduleShift = null;
						break;
					}
					...
					atsScheduleShift
							.setAttenceTime(DateFormatUtil.parseDate(o));
			...
			if (dateType.shortValue() == AtsConstant.DATE_TYPE_WORKDAY
					&& BeanUtils.isEmpty(atsScheduleShift.getShiftId())) {
				MsgUtil.addMsg(MsgUtil.WARN, "第" + (rowIndex + 1) + "行,"
						+ "班次名称为空值,该行导入失败");
				continue;
			}
	 ...
	 * @throws Exception
	 */
	private void handrimportExceltemplate1(HSSFSheet st, Short holidayHandle)
			throws Exception {
		...
		for (int rowIndex = 3; rowIndex <= st.getLastRowNum(); rowIndex++) {
			...
			if (BeanUtils.isEmpty(account)) {
				String v = "员工编号";
				MsgUtil.addMsg(MsgUtil.WARN, "第" + (rowIndex + 1) + "行," + v
						+ "为空值,该行导入失败");
				continue;
			}
			AtsAttendanceFile file = atsAttendanceFileService
			...
			for (int i = 0; i <= betweenDays; i++) {
				...
				String v = ExcelUtil.getCellFormatValue(cell);
				...
				MsgUtil.addMsg(MsgUtil.SUCCESS, "第" + (rowIndex + 1) + "行，"
						+ day + "日成功导入");
				this.saveData(atsScheduleShift);
			}
		}
	 ...
	 * @param holidayName
	 */
	private void holidayHandle(AtsScheduleShift atsScheduleShift,
			Short holidayHandle, String holidayName) {
		if (holidayHandle.shortValue() == AtsConstant.HOLIDAY_HANDLE_REPLACE
				&& BeanUtils.isNotEmpty(atsScheduleShift.getAttencePolicy())) {
			holidayName = this.getHoliday(atsScheduleShift.getAttencePolicy(),
					atsScheduleShift.getAttenceTime());
		}
