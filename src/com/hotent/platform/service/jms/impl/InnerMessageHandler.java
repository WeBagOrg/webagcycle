...
public class InnerMessageHandler implements IMessageHandler {
	...
	private String getContent(MessageModel model) {
		...
		try {
			...
			// 发送内容
			content = ServiceUtil.replaceTemplateTag(this.getTemplate(model), model.getReceiveUser().getFullname(), this.getSendUserName(model), model.getSubject(), url, model.getOpinion(), false);
