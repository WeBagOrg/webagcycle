...
FormDef.hasExistItem = function(fieldName){
	var itemName
	if($(controls[fieldName])[0].nodeName == 'INPUT'){
		 itemName = $(controls[fieldName])[0].name;
		if(!itemName)itemName = $(controls[fieldName])[0].attr["name"];
	}
	else var itemName = $(controls[fieldName]).children().eq(0).attr("name");
	var item = $("iframe:not(:hidden)").contents().find("[name='"+itemName+"']");
...
FormDef.insertHtml = function(editor,node){
	if (node.fieldType){
		...
		if (!controls[node.fieldName]){
			...
			if(FormDef.hasExistItem(node.fieldName)) return;
