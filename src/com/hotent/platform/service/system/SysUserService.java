...
/**
 * 对象功能:用户表 Service类 开发公司:广州宏天软件有限公司 开发人员:heyifan 创建时间:2012-12-17 16:02:46
 */
@Service
public class SysUserService extends BaseService<SysUser> implements ISysUserService {
	...
	public List<SysUser> getByCompanyRole(Long companyId, Long roleId) {
		return dao.getByCompanyRole(companyId, roleId);
	}
	...
	public List<ISysUser> getByGroup(Long groupId, String groupType) {
		List<SysUser> users = new ArrayList<SysUser>();
		if (TaskExecutor.USER_TYPE_ROLE.equals(groupType)) {
			users = dao.getByRoleId(groupId);
		} else if (TaskExecutor.USER_TYPE_ORG.equals(groupType)) {
			users = dao.getByOrgId(groupId);
		} else if (TaskExecutor.USER_TYPE_POS.equals(groupType)) {
			users = dao.getByPosId(groupId);
		}
	 ...
	 * @return
	 */
	public List<SysUser> findLinkMan(String email, Long userId) {
