...
/*	@ResponseBody
	...
	public ModelAndView preview(HttpServletRequest request,
		...
		if (formDefId > 0) {
			...
			permission = bpmFormRightsService.getByFormKeyAndUserId(bpmFormDef.getFormKey(), ContextUtil.getCurrentUserId(),"", "",platformType);
	...
	public ResultMessage initRights(HttpServletRequest request,
		...
		try {
			...
			String parentActDefId = RequestUtil.getString(request, "parentActDefId", "");
			if(StringUtil.isEmpty(parentActDefId)){
				bpmFormRightsService.deleteRight(formKey,actDefId,nodeId,platformType);
			}else{
				bpmFormRightsService.deleteRight(formKey,actDefId,nodeId,parentActDefId,platformType);
			}
	...
	public void exportXml(HttpServletRequest request,
		...
		map.put("sysBusEvent",
				RequestUtil.getBoolean(request, "sysBusEvent"));
		map.put("formDefTree", 
				RequestUtil.getBoolean(request, "formDefTree"));
	...
	public void setCategory(HttpServletRequest request,
		...
		if (categoryId == 0L) {
			...
			writeResultMessage(writer, new ResultMessage(ResultMessage.Fail,
					getText("controller.bpmFormDef.setCategory.notCategoryId")));
			return;
		}
		
		if (StringUtil.isEmpty(formKeys)) {
			writeResultMessage(writer, new ResultMessage(ResultMessage.Fail,
					getText("controller.bpmFormDef.setCategory.notSelectForm")));
			return;
		}
