		...
		<c:choose>
			<c:when test="${ isManage==0 }">
				<c:if test="${fn:indexOf(bpmNodeSet.jumpType,'1')!=-1}">
					<span style="height:30px;line-height:30px;"><input type="radio" <c:if test="${!isHandChoolse}"> checked='checked' </c:if> name="jumpType" onclick="chooseJumpType(1)" value="1" />&nbsp;正常跳转</span>
				</c:if>
				<c:if test="${fn:indexOf(bpmNodeSet.jumpType,'2')!=-1}">
					<span style="height:30px;line-height:30px;"><input type="radio" <c:if test="${isHandChoolse}"> checked='checked' </c:if> name="jumpType" onclick="chooseJumpType(2)" value="2" />&nbsp;选择路径跳转</span>
				</c:if>
