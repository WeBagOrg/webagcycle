...
function getOptionData1(){
	...
	$("tr.editable-tr",$("#option-table")).each(function(){
		...
		optionAry.push({key:optionKey,value:optionValue,checkboxDefault:checkboxDefault,radioDefault:radioDefault});
	});
	return optionAry;
};
...
function chrachange(controlType){
	if(controlType=="13"){
		$("input[name='radioDefault']").hide();
		$("input[name='checkboxDefault']").show();
	}else if(controlType=="14"){
		$("input[name='checkboxDefault']").hide();
		$("input[name='radioDefault']").show();
	}
}
...
function addColumn1(data){
    ...
    if(typeof(data.key)!= undefined && data.key!=null && data.key!=''){
		...
		if(data.radioDefault=='1'){
			$("input[name='radioDefault']",newTr).attr("checked","checked");
		}
		if(data.checkboxDefault=='1'){
			$("input[name='checkboxDefault']",newTr).attr("checked","checked");
		}
	}
	return newTr;
};
