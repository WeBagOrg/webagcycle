...
<#if field.fieldType == "varchar"><#---字符串类型-->
	<#switch field.controlType>
		...
		<#break>
		<#case 3><#--数据字典-->
				<input lablename="${field.fieldDesc}" class="dicComboTree" nodeKey="${field.dictType}" value="${r"${"}${table.variable.classVar}.${field.fieldName}${r"}"}" validate="{empty:false<#if field.isRequired == 1>,required:true</#if><#if field.isWebSign == 1>,isWebSign:true</#if>}" name="${field.fieldName}" height="200" width="125"/>
		<#break>
		<#case 4><#--人员选择器(单选)-->
		...
		<#case 11><#--下拉选项-->
			<span name="editable-input"  style="display:inline-block;padding:2px;" class="selectinput">
				<select name="${field.fieldName}" lablename="${field.fieldDesc}" controltype="select" validate='{empty:false<#if field.isRequired == 1>,required:true</#if><#if field.isWebSign == 1>,isWebSign:true</#if>}' data="${r"${"}${table.variable.classVar}.${field.fieldName}${r"}"}">
		...
		<#case 13><#--复选框-->
				<#list field.aryOptions?keys as optkey>
					<label><input type="checkbox" data="${r"${"}${table.variable.classVar}.${field.fieldName}${r"}"}" name="${field.fieldName}" value="${optkey}" validate="{<#if field.isRequired == 1>required:true</#if>}"/>${field.aryOptions[optkey]}</label>
				</#list>
		<#break>
		<#case 14><#--单选按钮-->
				<span>
				<#list field.aryOptions?keys as optkey>
					<label><input data="${r"${"}${table.variable.classVar}.${field.fieldName}${r"}"}" type="radio" name="${field.fieldName}" value="${optkey}" lablename="${field.fieldDesc}" validate="{<#if field.isRequired == 1>required:true</#if>}"/>${field.aryOptions[optkey]}</label>
				</#list>
				</span>
		<#break>
		<#case 15><#--日期控件-->
