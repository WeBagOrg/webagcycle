...
/**
 * 对象功能:用户条件配置 控制器类 开发公司:广州宏天软件有限公司 开发人员:ray 创建时间:2012-12-31 15:26:17
 */
@Controller
...
public class BpmUserConditionController extends BaseController {
	 ...
	 */
	@RequestMapping("save")
	@Action(description = "添加或更新用户条件配置", detail = "<#if 0==conditionId!0>添加<#else>更新</#if>" + "流程定义【${SysAuditLinkService.getBpmDefinitionLink(actDefId)}】节点【${SysAuditLinkService.getNodeName(actDefId,nodeId)}】的用户条件配置 ")
	public void save(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String conditionShow = request.getParameter("conditionShow");// 在列表中的展示内容
	 ...
	 */
	@RequestMapping("del")
	@Action(description = "删除用户条件配置 ", execOrder = ActionExecOrder.BEFORE, detail = "<#list StringUtils.split(id,\",\") as item>" + "<#assign entity=bpmUserConditionService.getById(Long.valueOf(item))/>" + "删除流程定义【${SysAuditLinkService.getBpmDefinitionLink(entity.actdefid)}】" + "节点【${SysAuditLinkService.getNodeName(entity.actdefid,entity.nodeid)}】的用户条件配置 " + "</#list>")
	...
	@ResponseBody
	@Action(description = "删除消息抄送中的用户条件配置 ", execOrder = ActionExecOrder.BEFORE, detail = "删除" + "<#list StringUtils.split(id,\",\") as item>" + "<#assign entity=bpmUserConditionService.getById(Long.valueOf(item))/>" + " 流程定义【${SysAuditLinkService.getBpmDefinitionLink(entity.actdefid)}】" + "<#if StringUtils.isNotEmpty(entity.nodeid)>" + "节点【${SysAuditLinkService.getNodeName(entity.actdefid,entity.nodeid)}】" + "</#if>" + "的用户条件配置 " + "</#list>")
	public String delByAjax(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			jsonObject.accumulate("result", true).accumulate("message", "删除用户条件配置 成功!");
		} catch (Exception ex) {
			String failMsg = "删除失败" + ex.getMessage();
			jsonObject.accumulate("result", false).accumulate("message", failMsg);
		}
		return jsonObject.toString();
	}

	@RequestMapping("updateGroup")
	@Action(description = "添加或更新用户条件配置 分组号", detail = "更新" + "<#list StringUtils.split(conditionIds,\",\") as item>" + "<#assign entity=bpmUserConditionService.getById(Long.valueOf(item))/>" + "流程定义【${SysAuditLinkService.getBpmDefinitionLink(entity.actdefid)}】" + "节点【${SysAuditLinkService.getNodeName(entity.actdefid,entity.nodeid)}】的用户条件配置 的分组号" + "</#list>")
