...
public class OpenfireMessageHandler implements IMessageHandler {
	...
	private void getSystemNotityChatManager() {
		...
		/*
		 * catch (SaslException e) { e.printStackTrace(); } catch (XMPPException e) { e.printStackTrace(); } catch (SmackException e) { e.printStackTrace(); } catch (IOException e) { e.printStackTrace(); }
		 */
	}
	...
	private String getContent(MessageModel model) {
		...
		// 发送内容
		content = ServiceUtil.replaceTemplateTag(this.getTemplate(model), model.getReceiveUser().getFullname(), this.getSendUserName(model), model.getSubject(), url, model.getOpinion(), true);
	...
	public static void main(String[] args) throws Exception {
		/*
		 * XMPPTCPConnectionConfigura config = XMPPTCPConnectionConfiguration.builder() .setDebuggerEnabled(true) .setUsernameAndPassword("zhangs", "1") .setResource("bpmx3") .setServiceName("vansai").build();
		 */
