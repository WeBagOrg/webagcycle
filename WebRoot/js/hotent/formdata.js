(function ($) {
	$.extend($.fn, {
		...
		handleFieldName:function(){
			form=$(this);
			form.find('[name*=":"]').each(function(){//筛选name属性含有:的元素
				var obj = $(this);
		...
		sortList:function(){
			form=$(this);
			form.find('[type="subtable"]').each(function(){//对子表记录进行排序，并设置编号
				 var tableId=this.id;//与主表类含有的子表类的属性（arrayList）值匹配
				 var subTable=$(this);
				 var index=0;
				 ...
				 subTable.find('[type="subdata"]').each(function(){
					 $(this).find("input,select,textarea").each(function(){
						 var name=$(this).attr('name');
						 var sortName=tableId+'List'+'['+index+'].'+name;//构造成可以封装成主表记录的name属性值，并且属性值为List结尾
