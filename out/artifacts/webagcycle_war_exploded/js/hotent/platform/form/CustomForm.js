$(function() {
	CustomForm = {
		...
		subTablePermission:function(obj,subFileJsonList,subTableShowList){
			$('div[type=subtable]').each(function() {
				...
				row.css('display', 'none').html('');//将edit的 formType 内容清空
				//窗口编辑模式
				if(form) {
					//修复窗口模式附件展示
	...
	/*	add : function(table, beforeElement) {
		...
		*/
		...
		add:function(table,beforeElement)//添加事件
		{
				var arg=arguments;
				var self=this;
				var right=table.attr("right");
				...
				else
				{
				  ...
				  if(arg.length>2){
					  var jsondata=arg[2];
					  /*var obj = row.find("input");
					  obj.each(function(){
					      var currName = $(this).attr("name").split(":");
						  if (currName.length > 2){
	                          $(this).val(eval("jsondata."+currName[2]));
						  } else {
							  $(this).val(eval("jsondata."+currName[0]));
						  }
					  });
					  //遍历row,获得select
					  obj = row.find("select");
					  ...
					  var obj = row.find("input,select");
					  obj.each(function(){
					      var currName = $(this).attr("id");
					      $(this).val(eval("jsondata."+currName));						  
					  });
					  this._add(table,row,beforeElement);
				  }
				  else{
		...
		getData: function() {
			...
			$('div[type=subtable][right=w || right=b][show=true||undefined]').each(function() {
				...
				$(this).find('[formtype]').not(":first").each(function() {	/*修复需要提交隐藏的行数据的bug*/	
					...
					//设置富文本框数据
					$("textarea[name^='s:'].myeditor",objRow).each(function(num) {
						var name = $(this).attr('name').replace(/.*:/, '');
						var data=getEditorData(editor[$("textarea.myeditor").index($(this))]);
						//var value=self.handNumberData(this);
						row[name] = data;
					});
			...
			if ((needEdit && this.menuWithEdit) || (!needEdit && this.menu)) {
				...
				var items = [ {
					...
					click : function() {
						...
						if(t.find("textarea.myeditor").length>0){
							self.delUeditor(t);
						}
	  ...
	  */
	  ...
	  //初始化子表选择器
		...
		//初始ueditor组件
	  initUeditor:function(table,row){
	  	if(editor){
	  		//
	  		if(!row||row==null){//向后插入
	  			var row =table.find('[formType=edit]').last();
	  			row.find("textarea[name^='s:'].myeditor").each(function(num) {
		  			//先判断是否存在，如果已存在，则不重新加载
		  			if($("textarea.myeditor").length==editor.length&&$("textarea[name^='s:'].myeditor").length>0){
		  				editor[$(this).index()].render(this);
		  			}else if($("textarea.myeditor").length>editor.length){
	  			...
	  			var index=table.find("[formType]:visible").index(row);//这里的row是获取的后面的行，不是待插入的行，待插入的行在该行前面插入
	  			if(index!=-1){
	  				//初始化ueditor，并翻入到editor数组中
	  				row=row.prev();
	  				row.find("textarea[name^='s:'].myeditor").each(function(num) {
	  					var ue =new baidu.editor.ui.Editor({minFrameHeight:300,initialFrameWidth:800,lang:'zh_cn'});
	  					ue.render(this);
	  					//editor.push(ue);
	  					editor.splice(index-1+num,0,ue);
	  				});
	  			}
	  		}
	  	}
	  ...
	  //删除ueditor
	  delUeditor:function(t){
	  	if(editor){
	  		if(t){
	  			t.find("textarea.myeditor").each(function(num) {
	  				editor.splice($("textarea.myeditor").index($(this)),1);
				});
	  		}
	  	}
	  },
