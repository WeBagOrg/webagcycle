			...
			<div class="hide-panel">
				<div class="panel-top">
					<div class="tbar-title">
						<span class="tbar-label">流程运行日志管理列表</span>
					</div>
					<div class="panel-toolbar">
						<div class="toolBar">
							...
							<c:if test="${runId>0}">
								<div class="group"><a class="link back" onclick="javascript:history.back()"><span></span>返回</a></div>
							</c:if>
						</div>	
					</div>
					<c:if test="${runId==0}">
						<div class="panel-search">
								<form id="searchForm" method="post" action="list.ht">
										<ul class="row">
											<li><span class="label">流程标题:</span><input type="text" name="Q_processSubject_SL"  class="inputText"  value="${param['Q_processSubject_SL']}"/></li>
											<li><span class="label">用户名称:</span><input type="text" name="Q_username_SL"  class="inputText"  value="${param['Q_username_SL']}"/></li>
											<li><span class="label">操作类型:</span>
											...
											<div class="row_date">
											<li><span class="label">操作时间 从:</span> <input  name="Q_begincreatetime_DL"  class="inputText date"  value="${param['Q_begincreatetime_DL']}"/></li>
				...
				<div class="panel-body">
					    ...
					    <display:table name="bpmRunLogList" id="bpmRunLogItem" requestURI="list.ht" sort="external" cellpadding="1" cellspacing="1" export="true"  class="table-grid except ">
							<display:column title="${checkAll}" media="html" style="width:30px;">
								  	<input type="checkbox" class="pk" name="id" value="${bpmRunLogItem.id}">
							</display:column>
							<display:column  title="流程标题"  style="width:180px;" sortable="true" sortName="processSubject">
								<c:choose>
									<c:when test="${bpmRunLogItem.operatortype eq 3}">
										${bpmRunLogItem.processSubject}
									</c:when>
									<c:otherwise>
								        <a href="#${bpmRunLogItem.runid}" onclick="FlowDetailWindow({runId:${bpmRunLogItem.runid}})" style="text-align:left;" >${bpmRunLogItem.processSubject}</a>
									</c:otherwise>
								</c:choose>
							</display:column>
							<display:column property="username" title="用户名称" style="width:80px;" sortable="true" sortName="username"></display:column>
							...
							<display:column title="操作类型" sortable="true" style="width:150px;" sortName="operatortype">
								<c:choose>
									...
									<c:when test="${bpmRunLogItem.operatortype eq 23}"><span class="green">删除流程草稿</span></c:when>
									<c:when test="${bpmRunLogItem.operatortype eq 18}"><span class="green">保存流程草稿</span></c:when>
							...
							<display:column title="管理" media="html" style="width:180px">
								<a href="del.ht?id=${bpmRunLogItem.id}" class="link del">删除</a>								
