	...
	public void codegen(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			for (String formDefId : formDefIds) {
				...
				List<String> fileList = genCode(basePath, system, tables, templateIds, override, flowKey, bpmFormDef,false);
...
//	}
	...
	//TODO！！！整理一下垃圾代码
	private List<String> genCode(String basePath, String system, List<BpmFormTable> tables, String[] templateIds, int override, String flowKey, BpmFormDef bpmFormDef,boolean underline) throws Exception {
		List<String> fileList = new ArrayList<String>();
		for (int i = 0; i < templateIds.length; i++) {
			Long templateId = Long.parseLong(templateIds[i]);
			SysCodeTemplate template = sysCodeTemplateService.getById(templateId);
			for (BpmFormTable table : tables) {
				...
				//格式化model中的字段名称
				if(underline){
					if(model!=null&&model.get("table")!=null){
						if(templatePath.indexOf("mapper.ftl")!=-1){
							formatFieldName(((BpmFormTable)model.get("table")).getFieldList(),false);
						}else{
							formatFieldName(((BpmFormTable)model.get("table")).getFieldList(),true);
						}
					}
				}
	...
	public void codegenZip(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String flowKey = RequestUtil.getString(request, "defKey");
		String[] formDefIds = RequestUtil.getString(request, "formDefIds").split(",");
		boolean undeline=RequestUtil.getBoolean(request, "underLine",false);
		// 自定义表相关
		...
		try {
			for (String formDefId : formDefIds) {
				...
				// 生成文件
				genCode(basePath, system, tables, templateIds, 1, flowKey, bpmFormDef,undeline);
			}
			// 先删除掉已经压缩的文件
	...
	//add ipph 2015-12-2
	private void formatFieldName(List<BpmFormField> fileds,boolean flag){
		if(null!=fileds&&fileds.size()>0){
			for(BpmFormField field:fileds){
				if(field.getFieldName().indexOf(StringUtil.UNDERLINE)!=-1&&flag){
					String fieldName=StringUtil.underlineToCamel(field.getFieldName().toLowerCase());
					field.setFieldName(fieldName);
				}else if(!flag){
					if(field.getFieldName().equalsIgnoreCase("id")) continue;
					String fieldName=StringUtil.camelToUnderline(field.getFieldName());
					field.setFieldName(fieldName);
				}
			}
		}
