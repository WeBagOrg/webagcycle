...
FormUtil.initEventBtn=function(scope){
	...
	eventBtn.on('click', function(){
		...
		FormUtil.executeQuery(queryArr,me,true);
	});
	// 将需要绑定联动的设置解析出来并绑定
			...
			targetInput.on("keydown change", function(event){
				...
				window.setTimeout(function(){
					...
					for(var queryJsonArr;queryJsonArr=currentQueryQueue.pop();){
						...
						//如果存在自定义查询,执行后置时间
						if(currentQueryQueue.length ==0){
							if(window.CustomQueryAftEvent__)CustomQueryAftEvent__();
						}
					}
...
FormUtil.executeQuery = function(queryArr,me,isclick){
	...
	for(var q=0;q<queryArr.length;q++){
		...
		if(isclick && isclick ==true) continue; //如果自定义对话框点击时，不触发自定义查询
...
FormUtil.pushDataToForm = function(data,fields,isToLowerCase,isAddRow,me,isDelSubTableBeforInsert){
	...
	for(var i=0;i<fields.length;i++){
		var json=fields[i];
		var src=json.src;
		if(isToLowerCase && src) src = src.toLowerCase();
...
FormUtil.setInputVal = function(inputObj,value){
	...
	var classvalue=$(inputObj).attr('class');
	var typevalue=$(inputObj).attr('type');
	if(!$.isEmpty(controltype) && controltype=='attachment'){
		var attachDiv = inputObj.prev();
		AttachMent.insertHtml(attachDiv,value); 
	}else{
		if(classvalue=='myeditor validError')//富文本
		{	
		  
		  /*if($(inputObj).attr("name")=='m:ip_proposal_flow:APPLYREMARK')
		      {
			  editor[0].setContent(value);}
		  else
		  { editor[1].setContent(value);  }*/
			//回填富文本，设置为不可编辑
		  if(typeof($(inputObj).attr("index"))!="undefined"){		  
		  	editor[$(inputObj).attr("index")].setContent(value);		  	
			editor[$(inputObj).attr("index")].setDisabled();
		  }
		}
		
		if(typevalue=='radio')//单选
		{	
...
FormUtil.exeCommonDialog=function(targetObj, json){
	...
	if(!queryArr==false && queryArr.length>0){
		for(var i=0;i<queryArr.length;i++){
			...
			var triggerInputValue;
			if(triggerInput.attr("type")=="radio"){
				triggerInputValue=$("[name$=':"+queryArr[i].trigger+"']:checked",$(preSelector)).val();
			}else if(triggerInput.attr("type")=="checkbox"){
				//如果是复选框的话
				triggerInputValue=$("[name$=':"+queryArr[i].trigger+"']:checked",$(preSelector));
				var arryValue =[];
				$(triggerInputValue).each(function (num,item){
					var str ="'"+item.value+"'";
					arryValue.push(str);
				});
				triggerInputValue=arryValue.join(",");
			}
			else{
				triggerInputValue = triggerInput.val();
			}
		...
		CommonDialog(name,function(data){
			...
			if(window.checkDataBeforeInsert){
				if(fields.length >0 && data){
					var targets ;
					for(var q=0,field;field=fields[q++];){
