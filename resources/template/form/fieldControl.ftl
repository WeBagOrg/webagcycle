...
<#macro input field>
			...
			<#switch field.controlType>
				...
				<#case 9><#--文件上传-->
						<div  name="div_attachment_container"  >
							...
							<textarea style="display:none" controltype="attachment" name="${field.fieldName}" lablename="${field.fieldDesc}" validate="{<#if field.isRequired == 1>required:true</#if>}" validatable="true" ></textarea>
							<a href="javascript:;" field="${field.fieldName}" class="link selectFile" atype="select"  onclick="{<#if field.getPropertyMap().isDirectUpLoad==1>AttachMent.directUpLoadFile(this);<#else>AttachMent.addFile(this);</#if>}" >选择</a>
						</div>	
				<#break>
				<#case 10><#--富文本框-->
				...
				<#case 13><#--复选框-->
						<#list field.aryOptions?keys as optkey>
							<label><input type="checkbox" name="${field.fieldName}" value="${optkey}" <#if field.checkboxDefault[optkey]==1> checked='checked'</#if>  validate="{<#if field.isRequired == 1>required:true</#if>}"/>${field.aryOptions[optkey]}</label>
						</#list>
				<#break>
				<#case 14><#--单选按钮-->
						<#list field.aryOptions?keys as optkey>
							<label><input type="radio" name="${field.fieldName}" value="${optkey}" <#if field.radioDefault[optkey]==1> checked</#if> lablename="${field.fieldDesc}" validate="{<#if field.isRequired == 1>required:true</#if>}"/>${field.aryOptions[optkey]}</label>
						</#list>
				<#break>
				<#case 15><#--日期控件-->
		...
		<#else>
			<#if  field.controlType == 16><#---隐藏域-->
				<input name="${field.fieldName}" type="hidden"  value="" validate="{empty:false<#if field.isRequired == 1>,required:true</#if><#if field.isWebSign == 1>,isWebSign:true</#if>}">
			<#elseif field.controlType == 10><#--富文本框editor-->
