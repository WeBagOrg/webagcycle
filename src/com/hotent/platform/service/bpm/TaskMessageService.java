...
public class TaskMessageService {
	...
	private void sendMsg(Task task, Map<String, String> map, SysUser user, String informTypes, String parentActDefId, BpmNodeSet bpmNodeSet, String subject, String opinion) throws Exception {
		...
		if (processCmd != null) {
			...
			bpmFormData = bpmFormHandlerDao.getByKey(bpmFormTable.getTableId(), processRun.getBusinessKey(), false);
		}
		...
		for (String infoType : infoTypeArray) {
			...
			messageModel.setSendUser((ISysUser) curUser);
	...
	/**
	   ...
	   * @throws Exception
	*/
	public void sendMessageInner(SysUser receiverUser, String title, String subject, String tempStr, SysUser sendUser, String cause, Long id, boolean isTask)
	throws Exception
	{
		...
		if (BeanUtils.isNotEmpty(id)) {		    	
			try {
				url = AppUtil.getServletContext().getContextPath();		
			} catch (Exception e) {
				e.printStackTrace();
			}		    		
			url = url + "/platform/ip/note/get.ht?flag=1&id=" + id;
		}//获取通知公告的地址
	    ...
	    title = ServiceUtil.replaceTitleTag(title, receiverUser.getFullname(), sendUser.getFullname(), subject, cause);

	    String content = ServiceUtil.replaceTemplateTag(tempStr, receiverUser.getFullname(), sendUser.getFullname(), subject, url, cause, false);
	    ...
	    this.messageProducer.send(innerModel);
	}
}
