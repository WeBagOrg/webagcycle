...
public class BpmFormField extends BaseModel implements Cloneable {
	...
	// String[] ary = this.options.split("\n");
	// for(String opt : ary){
	// jarray.add(new JSONObject().accumulate("key", opt).accumulate("value",
	// opt));
	// }
	// }
	// String reStr = jarray.toString();
	...
	/**
	 * 复选框的默认选项，在生成表单模板解释上面用到
	 * 
	 * @return
	 */
	public Map<String, Object> getCheckboxDefault() {
		Map<String, Object> map = new LinkedHashMap<String, Object>();
		if (StringUtil.isEmpty(this.options)) {
			return map;
		}
		if (StringUtil.isJson(this.options)) {
			...
			for (Object obj : jArray) {
				...
				if (!(jObject.containsKey("key"))
						|| !(jObject.containsKey("checkboxDefault"))) {
					continue;
				}
				String key = jObject.getString("key");
				String checkboxDefault = jObject.getString("checkboxDefault");
				map.put(key, checkboxDefault);
			}
		}
		return map;
	}
	...
	/**
	 * 单选框的默认选项，在生成表单模板解释上面用到
	 */
	public Map<String, Object> getRadioDefault() {
		Map<String, Object> map = new LinkedHashMap<String, Object>();
		if (StringUtil.isEmpty(this.options)) {
			return map;
		}
		if (StringUtil.isJson(this.options)) {
			...
			for (Object obj : jArray) {
				JSONObject jObject = (JSONObject) obj;
				if (!(jObject.containsKey("key"))
						|| !(jObject.containsKey("radioDefault"))) {
					continue;
				}
				String key = jObject.getString("key");
				String radioDefault = jObject.getString("radioDefault");
				map.put(key, radioDefault);
			}
		}
		return map;
	}
	...
	/**
	 * 慎用，因为没有传是否外部表，那么会每次都查询一下table来获取是否外部表 建议使用getDbFieldName(boolean
	 * isExtTable) 返回字段在数据库的名字
	 * 
	 * @param isExtTable
	 * @return String
	 * @exception
	...
	public String getDbFieldName() {
		BpmFormTable bpmFormTable = AppUtil.getBean(BpmFormTableService.class)
				.getById(tableId);
