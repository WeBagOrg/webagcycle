...
import com.hotent.platform.service.ats.AtsUtils;
...
//					atsAttencePolicy.getAttenceCycle(), true);
		...
		try {
			...
			for (AtsAttendanceFile atsAttendanceFile : list) {
				...
				if(BeanUtils.isNotEmpty(atsAttendanceFile
						.getUserId()))
					orgNames = userPositionService.getOrgnamesByUserId(atsAttendanceFile
							.getUserId());
				j.accumulate("fileId", atsAttendanceFile.getId())
						...
						.accumulate("userName",
								BeanUtils.isEmpty(atsAttendanceFile
										.getUserName()) ? ""
										: atsAttendanceFile.getUserName())
						.accumulate(
								"account",
								BeanUtils.isEmpty(atsAttendanceFile
										.getAccount()) ? "" : atsAttendanceFile
										.getAccount())
						.accumulate(
								"orgName",orgNames);
				jary.add(j);
			...
			json.element("results", jary.toString())
					.element("records", page.getTotalCount())
	...
	private JSONObject getPageList(List<AtsAttenceCalculate> list,
			PageBean pageBean, int betweenDays, Date startTime) {
		JSONArray jary = new JSONArray();
		AtsAttenceCalculateSet atsAttenceCalculateSet = atsAttenceCalculateSetService
				.getDefault();
		for (AtsAttenceCalculate calculate : list) {
			...
			JSONObject json = new JSONObject();
			json.accumulate("fileId", fileId)
					.accumulate(
							"account",
							BeanUtils.isEmpty(calculate.getAccount()) ? ""
									: calculate.getAccount())
					.accumulate(
							"userName",
							BeanUtils.isEmpty(calculate.getUserName()) ? ""
									: calculate.getUserName())
					.accumulate(
							"orgName",
	...
	private String getSetDetail(AtsAttenceCalculate cal,
		...
		if (BeanUtils.isEmpty(atsAttenceCalculateSet)
			...
			return "实出勤时数:"
					+ (BeanUtils.isEmpty(cal.getActualAttenceHours()) ? 0 : cal
	...
	public ModelAndView calendar(HttpServletRequest request,
		...
		for (AtsAttenceCalculate cal : list) {
			...
			json.accumulate("title", getSetDetail(cal, atsAttenceCalculateSet))
					.accumulate("start",
							DateFormatUtil.formatDate(cal.getAttenceTime()))
	...
	private void setAbsentRecord(AtsAttenceCalculate calculate) {
		...
		for (int i = 0; i < jary.size(); i++) {
			...
			if (BeanUtils.isNotEmpty(onCardTime)
					&& BeanUtils.isNotEmpty(offCardTime)) {
	...
	public void exportReportDetail(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		...
		try {
			...
			if (action == 1 || action == 2) {
				String[] titleAry = new String[] { "姓名", "员工编号", "组织", "考勤日期",
						"应出勤时数", "实际出勤时数", "迟到次数", "迟到分钟", "早退次数", "早退分钟",
						"旷工次数", "旷工小时数" };
				...
				String[] titleAry = new String[] { "姓名", "工号", "组织", "考勤日期",
						"星期", "班次名称", "异常类型", "第一段上班", "第一段下班", "第二段上班",
						"第二段下班", "第三段上班", "第三段下班" };
				excel.row(0, 0).value(titleAry).font(new IFontEditor() {
					// 设置字体
					@Override
				...
				for (int i = 0; i < list.size(); i++) {
					...
					String orgNames = userPositionService
							.getOrgnamesByUserId(calculate.getUserId());
				

					String attenceType = AtsUtils.getAttenceType(
							calculate.getAttenceType(), "、",true);
					getAbsentRecord(calculate);
					List<Object> rowData = new ArrayList<Object>();
					rowData.add(calculate.getUserName());
					rowData.add(calculate.getAccount());
					rowData.add(orgNames);
					rowData.add(DateFormatUtil.formatDate(calculate
							.getAttenceTime()));
					rowData.add(DateUtil.getWeekOfDate(calculate
							.getAttenceTime()));
	...
	public void isDownload(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
	...
	@ResponseBody
	public JSONObject detailList(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		QueryFilter filter = new QueryFilter(request, true);
		String[] type = RequestUtil.getStringAryByStr(request, "type");
		Long orgPath = RequestUtil.getLong(request, "orgPath");
	 ...
	 * @return
	 */
	private JSONObject getPageDetailList(List<AtsAttenceCalculate> list,
			PageBean pageBean) {
		JSONArray jary = new JSONArray();
		for (AtsAttenceCalculate calculate : list) {
			JSONObject json = new JSONObject();
			String orgNames = userPositionService.getOrgnamesByUserId(calculate
					.getUserId());
			String attenceType = AtsUtils.getAttenceType(
					calculate.getAttenceType(), "、",true);
			getAbsentRecord(calculate);
			json.accumulate("id", calculate.getId())
					.accumulate(
							"account",
							BeanUtils.isEmpty(calculate.getAccount()) ? ""
									: calculate.getAccount())
					.accumulate(
							...
							BeanUtils.isEmpty(calculate.getUserName()) ? ""
									: calculate.getUserName())
					.accumulate("orgName",
							BeanUtils.isEmpty(orgNames) ? "" : orgNames)
					.accumulate(
							"attenceTime",
							DateFormatUtil.formatDate(calculate
									.getAttenceTime()))
					.accumulate("week",
							DateUtil.getWeekOfDate(calculate.getAttenceTime()))
					.accumulate("shiftName", BeanUtils.isNotEmpty(calculate.getShiftName())?calculate.getShiftName():"")
					.accumulate("attenceType", attenceType)
					.accumulate(
							"shiftTime11",
							BeanUtils.isEmpty(calculate.getAbsentRecord11()) ? ""
									: calculate.getAbsentRecord11())
					.accumulate(
							"shiftTime12",
							BeanUtils.isEmpty(calculate.getAbsentRecord12()) ? ""
									: calculate.getAbsentRecord12())
					.accumulate(
							"shiftTime21",
							BeanUtils.isEmpty(calculate.getAbsentRecord21()) ? ""
									: calculate.getAbsentRecord21())
					.accumulate(
							"shiftTime22",
							BeanUtils.isEmpty(calculate.getAbsentRecord22()) ? ""
									: calculate.getAbsentRecord22())
					.accumulate(
							"shiftTime31",
							BeanUtils.isEmpty(calculate.getAbsentRecord31()) ? ""
									: calculate.getAbsentRecord31())
					.accumulate(
							"shiftTime32",
							BeanUtils.isEmpty(calculate.getAbsentRecord32()) ? ""
									: calculate.getAbsentRecord32());
			jary.add(json);
		}
		JSONObject json = new JSONObject();
		json.element("results", jary.toString())
