...
CREATE TABLE BPM_PRINT_TEMPLATE  (
   ...
   TEMPLATE           CLOB,
   ALIAS                VARCHAR2(200),
   IS_PRINT_IN_FINISH   NUMBER(2) ,
   IS_PRINT_IN_FORM     NUMBER(2) ,
   PRIMARY KEY (ID)
);
...
COMMENT ON COLUMN SYS_ORG_TACTIC.ORG_TYPE IS
'组织级别';
...
'附件';
COMMENT ON COLUMN SYS_BULLETIN.STATUS IS
'状态';
...
CREATE TABLE SYS_QUERY_SQLDEF 
(
   ID                   NUMBER(18)           NOT NULL,
   NAME                 VARCHAR2(50),
   SQL_                 VARCHAR2(2000),
   DSNAME               VARCHAR2(50),
   BUTTON_DEF           VARCHAR2(1000),
   ALIAS                VARCHAR2(50),
   CATEGORYID           NUMBER(18),
   SUPPORT_TAB          SMALLINT,
   CONSTRAINT PK_SYS_QUERY_SQLDEF PRIMARY KEY (ID)
);

COMMENT ON COLUMN SYS_QUERY_SQLDEF.ID IS '主键';
COMMENT ON COLUMN SYS_QUERY_SQLDEF.NAME IS '名称';
COMMENT ON COLUMN SYS_QUERY_SQLDEF.SQL_ IS 'SQL定义';
COMMENT ON COLUMN SYS_QUERY_SQLDEF.DSNAME IS '数据源名称';
COMMENT ON COLUMN SYS_QUERY_SQLDEF.BUTTON_DEF IS '按纽定义';
COMMENT ON COLUMN SYS_QUERY_SQLDEF.ALIAS IS '别名';
COMMENT ON COLUMN SYS_QUERY_SQLDEF.CATEGORYID IS '分类ID';
COMMENT ON COLUMN SYS_QUERY_SQLDEF.SUPPORT_TAB IS'是否支持tab';



CREATE TABLE SYS_QUERY_METAFIELD 
(
   ID                   NUMBER(18)           NOT NULL,
   SQL_ID               NUMBER(18),
   NAME                 VARCHAR2(50),
   ...
   IS_VIRTUAL           SMALLINT,
   VIRTUAL_FROM         SMALLINT,
   RESULT_FROM_TYPE     VARCHAR2(10),
   RESULT_FROM          VARCHAR2(400),
   ALARM_SETTING        VARCHAR2(400),
   DATE_FORMAT          VARCHAR2(40),
   URL                  VARCHAR2(200),
   FORMATER             VARCHAR2(1000),
   CONTROL_CONTENT      VARCHAR2(400),
   SN 					NUMBER(4),
   WIDTH 				NUMBER(4),	
   CONSTRAINT PK_SYS_QUERY_METAFIELD PRIMARY KEY (ID)
);

COMMENT ON COLUMN SYS_QUERY_METAFIELD.ID IS '主键';
COMMENT ON COLUMN SYS_QUERY_METAFIELD.SQL_ID IS 'SQL_ID';
COMMENT ON COLUMN SYS_QUERY_METAFIELD.NAME IS '字段名';
COMMENT ON COLUMN SYS_QUERY_METAFIELD.FIELD_NAME IS '实际字段名';
COMMENT ON COLUMN SYS_QUERY_METAFIELD.FIELD_DESC IS '字段备注';
COMMENT ON COLUMN SYS_QUERY_METAFIELD.IS_SHOW IS '是否可见';
COMMENT ON COLUMN SYS_QUERY_METAFIELD.IS_SEARCH IS '是否搜索';
COMMENT ON COLUMN SYS_QUERY_METAFIELD.CONTROL_TYPE IS '控件类型';
COMMENT ON COLUMN SYS_QUERY_METAFIELD.DATA_TYPE IS '数据类型';
COMMENT ON COLUMN SYS_QUERY_METAFIELD.IS_VIRTUAL IS '是否衍生列';
COMMENT ON COLUMN SYS_QUERY_METAFIELD.VIRTUAL_FROM IS '衍生列来自列';
COMMENT ON COLUMN SYS_QUERY_METAFIELD.RESULT_FROM_TYPE IS '来自类型';
COMMENT ON COLUMN SYS_QUERY_METAFIELD.RESULT_FROM IS '衍生列配置';
COMMENT ON COLUMN SYS_QUERY_METAFIELD.ALARM_SETTING IS '报警设定';
COMMENT ON COLUMN SYS_QUERY_METAFIELD.DATE_FORMAT IS '日期格式';
COMMENT ON COLUMN SYS_QUERY_METAFIELD.URL IS '连接地址';
COMMENT ON COLUMN SYS_QUERY_METAFIELD.FORMATER IS '格式化函数';
COMMENT ON COLUMN SYS_QUERY_METAFIELD.CONTROL_CONTENT IS '控件内容';


CREATE TABLE SYS_QUERY_VIEW 
(
   ID                   NUMBER(18)           NOT NULL,
   SQL_ALIAS            VARCHAR2(50),
   NAME                 VARCHAR2(50),
   ALIAS                VARCHAR2(50),
   CONDITIONS           CLOB,
   FILTER_TYPE          SMALLINT,
   FILTER               CLOB,
   BUTTONS              VARCHAR2(1000),
   INIT_QUERY           SMALLINT,
   TEMPLATE             CLOB,
   SUPPORT_GROUP        SMALLINT,
   GROUP_SETTING        VARCHAR2(400),
   PAGE_SIZE            SMALLINT,
   SHOW_ROWS_NUM        SMALLINT,
   SN                   NUMBER(18),
   NEED_PAGE            SMALLINT,
   TEMPLATEALIAS        VARCHAR2(50),
   CONSTRAINT PK_SYS_QUERY_VIEW PRIMARY KEY (ID)
);

COMMENT ON COLUMN SYS_QUERY_VIEW.ID IS '主键';
COMMENT ON COLUMN SYS_QUERY_VIEW.SQL_ALIAS IS 'sql别名';
COMMENT ON COLUMN SYS_QUERY_VIEW.NAME IS '视图名称';
COMMENT ON COLUMN SYS_QUERY_VIEW.ALIAS IS '视图别名';
COMMENT ON COLUMN SYS_QUERY_VIEW.CONDITIONS IS '查询条件定义';
COMMENT ON COLUMN SYS_QUERY_VIEW.FILTER_TYPE IS '过滤器类型';
COMMENT ON COLUMN SYS_QUERY_VIEW.FILTER IS '过滤器';
COMMENT ON COLUMN SYS_QUERY_VIEW.BUTTONS IS '按纽定义';
COMMENT ON COLUMN SYS_QUERY_VIEW.INIT_QUERY IS '是否初始化查询';
COMMENT ON COLUMN SYS_QUERY_VIEW.TEMPLATE IS '模版定义';
COMMENT ON COLUMN SYS_QUERY_VIEW.SUPPORT_GROUP IS '是否支持分组';
COMMENT ON COLUMN SYS_QUERY_VIEW.GROUP_SETTING IS '分组设定';
COMMENT ON COLUMN SYS_QUERY_VIEW.PAGE_SIZE IS '分页大小';
COMMENT ON COLUMN SYS_QUERY_VIEW.SHOW_ROWS_NUM IS '显示行号';
COMMENT ON COLUMN SYS_QUERY_VIEW.SN IS '排序';
COMMENT ON COLUMN SYS_QUERY_VIEW.NEED_PAGE IS '是否分页';
COMMENT ON COLUMN SYS_QUERY_VIEW.TEMPLATEALIAS IS '摸版别名';


CREATE TABLE SYS_QUERY_FIELDSETTING 
(
   ID                   NUMBER(18)           NOT NULL,
   VIEW_ID              NUMBER(18),
   FIELD_NAME           VARCHAR2(50),
   SORT_ABLE            SMALLINT,
   DEFAULT_SORT         SMALLINT,
   SORT_SEQ             VARCHAR2(10),
   ALIGN                VARCHAR2(10),
   FROZEN               SMALLINT,
   SUMMARY_TYPE         VARCHAR2(50),
   SUMMARY_TEMPLATE     VARCHAR2(200),
   HIDDEN               SMALLINT,
   CONSTRAINT PK_SYS_QUERY_FIELDSETTING PRIMARY KEY (ID)
);

COMMENT ON COLUMN SYS_QUERY_FIELDSETTING.ID IS '主键';
COMMENT ON COLUMN SYS_QUERY_FIELDSETTING.VIEW_ID IS '视图ID';
COMMENT ON COLUMN SYS_QUERY_FIELDSETTING.FIELD_NAME IS '字段名称';
COMMENT ON COLUMN SYS_QUERY_FIELDSETTING.SORT_ABLE IS '允许排序';
COMMENT ON COLUMN SYS_QUERY_FIELDSETTING.DEFAULT_SORT IS '默认排序';
COMMENT ON COLUMN SYS_QUERY_FIELDSETTING.SORT_SEQ IS '默认排序方向';
COMMENT ON COLUMN SYS_QUERY_FIELDSETTING.ALIGN IS '对齐方式';
COMMENT ON COLUMN SYS_QUERY_FIELDSETTING.FROZEN IS '是否冻结';
COMMENT ON COLUMN SYS_QUERY_FIELDSETTING.SUMMARY_TYPE IS '计算类型';
COMMENT ON COLUMN SYS_QUERY_FIELDSETTING.SUMMARY_TEMPLATE IS '计算模版';
COMMENT ON COLUMN SYS_QUERY_FIELDSETTING.HIDDEN IS '是否隐藏';

/*==============================================================*/
/* Table: SYS_HISTORY_DATA                                      */
/*==============================================================*/
CREATE TABLE SYS_HISTORY_DATA 
(
   ID                   NUMBER(18,0)         NOT NULL,
   TYPE_                VARCHAR2(50),
   OBJ_ID               NUMBER(18,0),
   ...
   SUBJECT              VARCHAR2(200),
   CREATETIME           DATE,
   CONSTRAINT PK_SYS_HISTORY_DATA PRIMARY KEY (ID)
);

COMMENT ON COLUMN SYS_HISTORY_DATA.ID IS
'主键';

COMMENT ON COLUMN SYS_HISTORY_DATA.TYPE_ IS
'数据类型';

COMMENT ON COLUMN SYS_HISTORY_DATA.OBJ_ID IS
'关联对象ID';

COMMENT ON COLUMN SYS_HISTORY_DATA.CONTENT IS
'存储内容';

COMMENT ON COLUMN SYS_HISTORY_DATA.CREATOR IS
'创建人';

COMMENT ON COLUMN SYS_HISTORY_DATA.CREATETIME IS
'创建时间';


CREATE TABLE SYS_BULLETIN_TEMPLATE 
(
   ID                   NUMBER(20)           NOT NULL,
...
COMMENT ON COLUMN SYS_BULLETIN_TEMPLATE.ID IS
'主键';

COMMENT ON COLUMN SYS_BULLETIN_TEMPLATE.NAME IS
'名称';

COMMENT ON COLUMN SYS_BULLETIN_TEMPLATE.ALIAS IS
'别名';

COMMENT ON COLUMN SYS_BULLETIN_TEMPLATE.TEMPLATE IS
'html内容';

ALTER TABLE SYS_BULLETIN_TEMPLATE
   ADD CONSTRAINT BULLETIN_TEMPLETE_UNIQUE UNIQUE (ALIAS);

CREATE TABLE SYS_READ_RECORD 
(
   ID                   NUMBER(20)              NOT NULL,
   SOURCE               VARCHAR2(50),
   GROUPID              NUMBER(20),
   ...
   CREATETIME           DATE                 NOT NULL,
   CONSTRAINT PK_SYS_READ_RECORD PRIMARY KEY (ID)
);

COMMENT ON COLUMN SYS_READ_RECORD.SOURCE IS
'来源；eg:公告：BULLETINID';

COMMENT ON COLUMN SYS_READ_RECORD.GROUPID IS
'分组ID；删除时可以删除一组，如任务存放流程实例id,栏目ID';

COMMENT ON COLUMN SYS_READ_RECORD.OBJECTID IS
'对象ID';

COMMENT ON COLUMN SYS_READ_RECORD.USERID IS
'用户ID';

COMMENT ON COLUMN SYS_READ_RECORD.CREATETIME IS
'阅读时间';

CREATE TABLE SYS_PWD_STRATEGY 
(
   ID                   INTEGER              DEFAULT 0 NOT NULL,
   INIT_PWD             VARCHAR2(50)         DEFAULT NULL,
   FORCE_CHANGE_INIT_PWD SMALLINT             DEFAULT NULL,
   PWD_RULE             SMALLINT             DEFAULT NULL,
   PWD_LENGTH           SMALLINT             DEFAULT NULL,
   VALIDITY             SMALLINT             DEFAULT NULL,
   HANDLE_OVERDUE       SMALLINT             DEFAULT NULL,
   OVERDUE_REMIND       SMALLINT             DEFAULT NULL,
   VERIFY_CODE_APPEAR   SMALLINT             DEFAULT NULL,
   ERR_LOCK_ACCOUNT     SMALLINT             DEFAULT NULL,
   ENABLE_              SMALLINT             DEFAULT NULL,
   DESC_                CLOB,
   CONSTRAINT PK_SYS_PWD_STRATEGY PRIMARY KEY (ID)
);
