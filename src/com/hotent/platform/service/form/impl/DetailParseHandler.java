...
public class DetailParseHandler extends AbstractParseHandler {
	...
	private void handleField_table(Element element, Map<String, Object> param) {
		...
		if(null==bpmFormField){//如果form表单是在数据库中手动建立，并通过添加外部表加入到系统中，而表字段定义时使用下划线作为分隔符时，可能不能找到对应的BpmFormField,不能主动去掉下划线，变成首字母大写
			fieldName=element.attr("name").split(":")[2];
			bpmFormField = getByFieldNameFormList((List<BpmFormField>) param.get("fieldList"), fieldName);
		}
