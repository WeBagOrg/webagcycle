...
//			return;
	...
	private void getOpinionField(ProcessCmd processCmd, Long defId,String parentActDefId){
		List<BpmNodeSet> bpmNodeSets=null;
		List<String> list=new ArrayList<String>();
		if(StringUtil.isEmpty(parentActDefId)){
			bpmNodeSets=bpmNodeSetService.getByDefIdOpinion(defId);
		}
		else{
			bpmNodeSets=bpmNodeSetService.getByParentDefIdOpinion(defId, parentActDefId);
		}
	...
	private BpmFormData handlerFormData(ProcessCmd processCmd, ProcessRun processRun, String nodeId) throws Exception {
		...
		if (StringUtils.isEmpty(json)) {
			...
			if (StringUtil.isNotEmpty(opinionField) && batComplte!=null && batComplte) {
				...
				BpmFormData data = bpmFormHandlerDao.getByKey(bpmFormDef.getTableId(), processRun.getBusinessKey(), false);
	...
	public static void handFieldOpinion(ProcessCmd processCmd, BpmFormData bpmFormData) {
		...
		if(BeanUtils.isNotEmpty(opinionFields)){
			for(String field:opinionFields){
				if (!bpmFormData.isExternal()) {
					field = TableModel.CUSTOMER_COLUMN_PREFIX + field;
				}
				field=field.toLowerCase();
				if(!mainMap.containsKey(field)) continue;
