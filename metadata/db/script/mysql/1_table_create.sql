...
(
   ...
   MOBILEFORMKEY        VARCHAR(50) COMMENT '手机表单KEY',
...
(
   ...
   PRIVILEGEMODE        SMALLINT COMMENT '0=拥有所有特权
            ...
            2=允许一票制
            3=允许补签',
   USERTYPE             SMALLINT COMMENT '用户类型',
...
(
   ...
   ALIAS                VARCHAR(200),
   IS_PRINT_IN_FINISH   INT(2),
   IS_PRINT_IN_FORM     INT(2)
);
...
(
   ...
   FROMTYPE             SMALLINT COMMENT '数据来源',
   pwdUpdTime           DATETIME COMMENT '密码更新时间'
);
...
(
   ID                   NUMERIC(18,0) NOT NULL COMMENT '主键',
   VIEW_ID              NUMERIC(18,0) COMMENT '视图ID',
   FIELD_NAME           VARCHAR(50) COMMENT '字段名称',
   SORT_ABLE            SMALLINT COMMENT '允许排序',
   DEFAULT_SORT         SMALLINT COMMENT '默认排序',
   SORT_SEQ             VARCHAR(10) COMMENT '默认排序方向',
   ALIGN                VARCHAR(10) COMMENT '对齐方式',
   FROZEN               SMALLINT COMMENT '是否冻结',
   SUMMARY_TYPE         VARCHAR(50) COMMENT '计算类型',
   SUMMARY_TEMPLATE     VARCHAR(200) COMMENT '计算模版',
   HIDDEN               SMALLINT COMMENT '是否隐藏',
   PRIMARY KEY (ID)
);
...
(
   ID                   NUMERIC(18,0) NOT NULL COMMENT '主键',
   SQL_ID               NUMERIC(18,0) COMMENT 'SQL_ID',
   NAME                 VARCHAR(50) COMMENT '字段名',
   FIELD_NAME           VARCHAR(50) COMMENT '实际字段名',
   FIELD_DESC           VARCHAR(100) COMMENT '字段备注',
   IS_SHOW              SMALLINT COMMENT '是否可见',
   IS_SEARCH            SMALLINT COMMENT '是否搜索',
   CONTROL_TYPE         SMALLINT COMMENT '控件类型',
   DATA_TYPE            VARCHAR(10) COMMENT '数据类型',
   IS_VIRTUAL           SMALLINT COMMENT '是否衍生列',
   VIRTUAL_FROM         SMALLINT COMMENT '衍生列来自列',
   RESULT_FROM_TYPE     VARCHAR(10) COMMENT '来自类型',
   RESULT_FROM          VARCHAR(400) COMMENT '衍生列配置',
   ALARM_SETTING        VARCHAR(400) COMMENT '报警设定',
   DATE_FORMAT          VARCHAR(40) COMMENT '日期格式',
   URL                  VARCHAR(200) COMMENT '连接地址',
   FORMATER             VARCHAR(1000) COMMENT '格式化函数',
   CONTROL_CONTENT      VARCHAR(400) COMMENT '控件内容',
   SN					NUMERIC(4),
   WIDTH 				NUMERIC(4),
   PRIMARY KEY (ID)
);
...
(
   ID                   NUMERIC(18,0) NOT NULL COMMENT '主键',
   SQL_ALIAS            VARCHAR(50) COMMENT 'sql别名',
   NAME                 VARCHAR(50) COMMENT '视图名称',
   ALIAS                VARCHAR(50) COMMENT '视图别名',
   CONDITIONS           TEXT COMMENT '查询条件定义',
   FILTER_TYPE          SMALLINT COMMENT '过滤器类型',
   FILTER               TEXT COMMENT '过滤器',
   BUTTONS              VARCHAR(1000) COMMENT '按纽定义',
   INIT_QUERY           SMALLINT COMMENT '是否初始化查询',
   TEMPLATE             TEXT COMMENT '模版定义',
   SUPPORT_GROUP        SMALLINT COMMENT '是否支持分组',
   GROUP_SETTING        VARCHAR(400) COMMENT '分组设定',
   PAGE_SIZE            SMALLINT COMMENT '分页大小',
   SHOW_ROWS_NUM        SMALLINT COMMENT '显示行号',
   SN                   NUMERIC(18,0) COMMENT '排序',
   NEED_PAGE            SMALLINT COMMENT '是否分页',
   TEMPLATEALIAS        VARCHAR(50) COMMENT '摸版别名',
   PRIMARY KEY (ID)
);
...
(
   ID                   NUMERIC(18,0) NOT NULL COMMENT '主键',
   NAME                 VARCHAR(50) COMMENT '名称',
   SQL_                 VARCHAR(2000) COMMENT 'SQL定义',
   DSNAME               VARCHAR(50) COMMENT '数据源名称',
   BUTTON_DEF           VARCHAR(1000) COMMENT '按纽定义',
   ALIAS                VARCHAR(50) COMMENT '别名',
   CATEGORYID           NUMERIC(18,0) COMMENT '分类ID',
   SUPPORT_TAB          SMALLINT COMMENT '是否支持tab',
   PRIMARY KEY (ID)
);
...
(
   ID                   NUMERIC(18,0) NOT NULL COMMENT '主键',
   TYPE_                VARCHAR(50) COMMENT '数据类型',
   OBJ_ID               NUMERIC(18,0) COMMENT '关联对象ID',
   CONTENT              TEXT COMMENT '存储内容',
   CREATOR              VARCHAR(50) COMMENT '创建人',
   SUBJECT              VARCHAR(200),
   CREATETIME           DATETIME COMMENT '创建时间',
   PRIMARY KEY (ID)
...
(
   ...
   ALIAS                VARCHAR(20) DEFAULT NULL COMMENT '别名',
   TEMPLATE             TEXT COMMENT 'html内容'
);

ALTER TABLE SYS_BULLETIN_TEMPLATE
   ADD UNIQUE BULLETIN_TEMPLETE_UNIQUE (ALIAS);

   
CREATE TABLE SYS_READ_RECORD
(
   ID                   BIGINT NOT NULL,
   SOURCE               VARCHAR(50) COMMENT '来源；eg:公告：BULLETINID',
   GROUPID              BIGINT COMMENT '分组ID；删除时可以删除一组，如任务存放流程实例id,栏目ID',
   OBJECTID             BIGINT NOT NULL COMMENT '对象ID',
   USERID               BIGINT NOT NULL COMMENT '用户ID',
   CREATETIME           DATE NOT NULL COMMENT '阅读时间',
   PRIMARY KEY (ID)
);
...
(
   ...
   FORCE_CHANGE_INIT_PWD SMALLINT(6) DEFAULT NULL,
   PWD_RULE             SMALLINT(6) DEFAULT NULL,
   PWD_LENGTH           SMALLINT(6) DEFAULT NULL,
   VALIDITY             SMALLINT(6) DEFAULT NULL,
   HANDLE_OVERDUE       SMALLINT(6) DEFAULT NULL,
   OVERDUE_REMIND       SMALLINT(6) DEFAULT NULL,
   VERIFY_CODE_APPEAR   SMALLINT(6) DEFAULT NULL,
   ERR_LOCK_ACCOUNT     SMALLINT(6) DEFAULT NULL,
   ENABLE_              SMALLINT(6) DEFAULT NULL,
   DESC_                LONGTEXT,
   PRIMARY KEY (ID)
);
