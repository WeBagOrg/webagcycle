...
public class EditParseHandler extends AbstractParseHandler {
	...
	private void parseTable(Document doc, Map<String, Object> param) {
		...
		for (Element subtableElement : subtableElements) {
			...
			String subTableName = StringUtil.underlineToCamel(subTable.getTableName().toLowerCase());//子表驼峰名称
			
			subtableElement.attr("id", subTableName);// 设置id
			subtableElement.attr("tablename",subTableName);//修改真正类名
			...
			divElement.wrap("<c:forEach items='${" + subTableName + "List}' var='" + subTableName + "' varStatus='status'>");
	...
	private void handleField_table(Element element) {
		String tableName =StringUtil.underlineToCamel(element.attr("name").split(":")[1].toLowerCase());
		...
		}else if(element.attr("class").equals("Wdate")){//日期类型
			String val = "<fmt:formatDate value='"+"${" + tableName + "." + fieldName + "}"+"' pattern='yyyy-MM-dd'/>";
		...
		name=name.replace(element.attr("name").split(":")[1], tableName);
		name=name.replace(element.attr("name").split(":")[2], fieldName);
		element.attr("name",name);//替换字段名
	}
	...
	private void handleFormType(Map<String, Object> param) {
		...
		String tableName =StringUtil.underlineToCamel(subTable.getTableName());
		for (Element fieldElement : fieldElements) {
			String fieldName = StringUtil.underlineToCamel(fieldElement.attr("fieldname"));
			
			listRowElement.append("<td name=\"s:" + tableName + ":" + fieldName + "\">${" + tableName + "." + fieldName + "}</td> ");
			listRowElement.append("<td style='display:none;'><input name='s:" + tableName + ":" + fieldName + "' type=\"hidde\" value='${" + tableName + "." + fieldName + "}'/></td>");
			fieldElement.remove();
		}
		/** ---------------------->end **/
		...
		windowDivElement.attr("id", tableName + "Form");
	}
