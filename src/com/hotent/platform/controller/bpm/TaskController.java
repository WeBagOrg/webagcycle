...
import com.hotent.platform.ip.util.ThreadVariableUtil;
...
public class TaskController extends BaseController {
	 ...
	 * @throws Exception
	 */
	private FormModel getForm(BpmNodeSet bpmNodeSet, String businessKey, String actDefId, String ctxPath,boolean isCopyFlow) throws Exception {
		...
		if (bpmNodeSet.getFormType() == BpmConst.OnLineForm) {
			...
			if (StringUtil.isNotEmpty(formKey)) {
				...
				if (bpmFormDef != null) {
					...
					String formHtml = bpmFormHandlerService.obtainHtml(bpmFormDef, ContextUtil.getCurrentUserId(), businessKey, "", actDefId, bpmNodeSet.getNodeId(), ctxPath, "", isCopyFlow);
	...
	private Map<String,Object> getPageParam(HttpServletRequest request) {
	...
	public ModelAndView toStartFlow(HttpServletRequest request, HttpServletResponse response,@PathVariable(value="flowKey") String flowKey) throws Exception {
		...
		if(def == null) return ServiceUtil.getTipInfo("该流程定义已经被删除!");
	...
	public ModelAndView startFlowForm(HttpServletRequest request, HttpServletResponse response,Long defId) throws Exception {
		...
		// 构建参数到到JSP页面。
		Map<String,Object> paraMap = getPageParam(request);
		//ipph add将参数设置到自定义线程变量中
		//if(paraMap!=null&&paraMap.size()>0) ThreadVariableUtil.setThreadVariableMap(paraMap);
		...
		if (BeanUtils.isNotEmpty(processRun) && processRun.getStatus().equals(ProcessRun.STATUS_FORM)) {
			...
			if (StringUtil.isNotEmpty(processRun.getBusinessUrl())) {
				...
				form = bpmFormHandlerService.obtainHtml(bpmFormDef, sysUser.getUserId(), businessKey, "", actDefId, nodeId, ctxPath, "", false);
			}
			// 流程定义里面的启动
		} else {
			...
			FormModel formModel = getForm(bpmNodeSet, businessKey, actDefId, ctxPath,isCopyFlow);
			// 是外部表单
	...
	private ModelAndView getToStartView(HttpServletRequest request, HttpServletResponse response, ModelAndView mv, int isManage) throws Exception {
		...
		return mv.addObject("task", taskEntity).addObject("bpmNodeSet", bpmNodeSet).addObject("processRun", processRun).addObject("bpmDefinition", bpmDefinition).addObject("isSignTask", isSignTask).addObject("isCanBack", isCanBack).addObject("isCanAssignee", isCanAssignee).addObject("isHidePath", isHidePath).addObject("toBackNodeId", toBackNodeId).addObject("form", form).addObject("isExtForm", isExtForm).addObject("isEmptyForm", isEmptyForm).addObject("taskAppItems", taskAppItems).addObject("mapButton", mapButton).addObject("detailUrl", detailUrl).addObject("isManage", isManage).addObject("bpmGangedSets", gangedSetJarray).addObject("sysFile", sysFile).addObject("taskOpinion", taskOpinion).addObject("isHandChoolse", isHandChoolse).addObject("curUserId", sysUser.getUserId().toString())
				.addObject("curUserName", sysUser.getFullname()).addObject("hasGlobalFlowNo", hasGlobalFlowNo).addObject("formKey", bpmNodeSet.getFormKey());
	}
	...
	public ModelAndView getForm(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (bpmDefinition != null) {
			FormModel formModel = bpmFormDefService.getForm(processRun, taskName, userId, ctxPath, variables);
