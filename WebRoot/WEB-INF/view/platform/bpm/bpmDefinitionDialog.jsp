        ...
        <script type="text/javascript">
            	...
            	function initData(conf){
              	    if(conf!=undefined && conf!=null && conf!=""){      //模态窗口的接收方法
              	    	var defArry = conf.defArry;
              	    	defMark = conf.defMark;          // id 或者key做标识
              	    	for(i=0;i<defArry.length;i++){
                				add(defArry[i]);             	    		
                		}
              	    }
            	};
            	...
            	function selectFlow(){
            		...
            		var actDefIdArr =new Array();
            		$.each(chIds,function(i,ch){
            			var aryTmp=$(ch).val().split("#");
            			defIdArr.push(aryTmp[0]);
            			subjectArr.push(aryTmp[1]);
            			defKeyArr.push(aryTmp[2]);
            			actDefIdArr.push(aryTmp[3]);
            		});
            		var defIds=defIdArr.join(','),
            			subjects=subjectArr.join(','),
            			defKeys=defKeyArr.join(','),
            			actDefIds =actDefIdArr.join(',');
            		var obj={defIds:defIds,subjects:subjects,defKeys:defKeys,actDefIds:actDefIds};
            	...
            	function add(data) {
    				...
    				var defId=aryTmp[0];
    				//以key为标识时
    				if(defMark=="key"&&aryTmp.length>2){    
    					defId=aryTmp[2];
    				}
