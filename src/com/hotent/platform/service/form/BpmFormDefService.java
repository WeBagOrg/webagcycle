...
import com.hotent.platform.service.system.SysHistoryDataService;
...
public class BpmFormDefService extends BaseService<BpmFormDef> {
	...
	public void addForm(BpmFormDef bpmFormDef) throws Exception {
		...
		historyDataService.add(SysHistoryData.SYS_FORMDEF_TEMPLATE, bpmFormDef.getSubject(),
				bpmFormDef.getHtml(), bpmFormDef.getFormDefId());
	}
	...
	public void copyForm(BpmFormDef bpmFormDef, String oldFormkey) {
		...
		if (bpmFormDef.getDesignType() == 0) {
			List<BpmFormRights> list = bpmFormRightsDao.getByFormKeyNodeEmpty(oldFormkey, PlatformType.PC);
	 ...
	 * @throws Exception
	 */
	public FormModel getForm(ProcessRun processRun, String nodeId, Long userId, String ctxPath, Map<String, Object> variables) throws Exception {
	...
	public String getFormUrl(String taskId, Long defId, String nodeId, String businessKey, String ctxPath) {
		...
		if (StringUtil.isEmpty(formUrl)) {
			BpmNodeSet node = bpmNodeSetService.getBySetType(defId, BpmNodeSet.SetType_GloabalForm);
	...
	public Long getCurrentTableId(BpmNodeSet nodeSet) {
		...
		if (nodeSet.getFormType().equals(Short.parseShort("0"))) {
			...
			BpmNodeSet globalForm = bpmNodeSetDao.getBySetType(nodeSet.getDefId(), BpmNodeSet.SetType_GloabalForm);
			if (globalForm != null) {
	...
	private void exportBpmFormRights(String formKey, BpmFormDefXml bpmFormDefXml) {
		List<BpmFormRights> bpmFormRightsList = bpmFormRightsDao.getByFormKeyNodeEmpty(formKey, PlatformType.PC);
