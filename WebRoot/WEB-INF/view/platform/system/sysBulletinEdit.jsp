...
<head>
	...
	<script type="text/javascript">
		...
		function templateAdd(obj){
			var htmldata;
			$.ajax({
				type : "get",
				async : false,
				dataType : "json",
				url : __ctx + "/platform/system/sysBulletinTemplate/selector.ht?id="+obj.value,
				success : function(data) {
					htmldata =data.sysbulletintemplate.template;
				},
				error : function(data) {
					alert(data);
				}
			});
			var temContent = editor.getData();
...
<c:if test="${!isAddByFlow}">
	...
	<c:if test="${!isAddByFlow}"><form id="sysBulletinForm" method="post" action="save.ht"></c:if>
			<table class="table-detail" cellpadding="0" cellspacing="0" border="0">
				<tbody>
					...
					<tr>
						<th width="15%">公告模板: </th>
						<td>
							<select name="templateList" id="templateList" class="inputText" style="width: 250px;" onchange="templateAdd(this);">
		...
		<c:if test="${!isAddByFlow}"></form></c:if>
	</div>
</body>
</html>
